		...
		if (json.success) {
			...
			if (0 === +peepsodata.userid || +peepsodata.currentuserid === +peepsodata.userid) {
				...
				if (activity._action_repost_button) {
					...
					$button.html(peepsodata.label_done).css('color', 'green');
...
PsActivity.prototype.action_remove_link_preview = function(post_id, act_id) {
	...
	if ($act.length) {
		...
		$act.find('.ps-stream-body .ps-js-activity-content .ps-js-hide-url').show();
...
PsActivity.prototype.comment_save = function(act_id, elem) {
	...
	req = peepso.observer.applyFilters(
		'comment_req',
		{
			...
			last: jQuery(
				'.comment-container[data-act-id=' + act_id + '] .cstream-comment:last'
			).data('comment-id')
		},
		sel
	);
	...
	if ($ai.length > 0) {
		...
		peepso.postJson('activity.savecomment', req, function(json) {
			if (json.success) {
				...
				jQuery(
					'[data-comment-id=' + post_id + '] .cstream-content > .cstream-attachments'
				).html(json.data.attachments);
