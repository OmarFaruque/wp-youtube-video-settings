...
class PeepSoUser
{
	 ...
	 */
	private function get_name_parts($is_admin = false, $first_only = FALSE)
	{
		...
		// If we need only the first element, remove all others to avoid third party filters messing it up
		if($first_only) {
		    $new_name_parts = array();
		    $new_name_parts[] = $this->name_parts[0];
		    $this->name_parts = $new_name_parts;
        }

		$this->name_parts = apply_filters('peepso_get_name_parts', $this->name_parts);
	 ...
	 * Reason:		#812 refactoring of the name getters
	 */
	public function get_fullname($is_admin = FALSE)
	{
		$name_parts = $this->get_name_parts($is_admin);
	...
	{
		$name_parts = $this->get_name_parts(FALSE, TRUE);
